...
class Cinetpay:
    """Classe bouchon pour éviter l'erreur d'import"""
    def __init__(self, *args, **kwargs):
        pass

    def verify_payment(self, *args, **kwargs):
        return {"code": "00", "msg": "Succès simulé"}

    def get_payement_url(self, *args, **kwargs):
        return {"code": "201", "data": {"payment_url": "http://fake-payment-url.com"}}
...
class Panier(models.Model):
    ...
    @property
    def total(self):
        panier = Panier.objects.get(id=self.id) # type: ignore
        sum = 0
        for i in panier.produit_panier.all(): # type: ignore
            sum = sum + i.total
        return int(sum)
    ...
    def check_empty(self):
        panier = Panier.objects.get(id=self.id) # type: ignore
        if panier.produit_panier.count() > 0: # type: ignore
            return True
        else:
            return False
