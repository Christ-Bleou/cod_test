...
# On simule la librairie manquante pour permettre au projet de tourner
class Cinetpay:
    ...
    def verify_payment(self, *args, **kwargs):
        ...
        return {"code": "00", "msg": "Succès simulé par le Mock"}
        
    def get_payement_url(self, *args, **kwargs):
        # On simule une URL de paiement fictive
        return {"code": "201", "data": {"payment_url": "http://mock-payment-url.com"}}
...
def product_detail(request, slug):
    ...
    produits = Produit.objects.filter(categorie=produit.categorie).exclude(id=produit.id)[:3] # type: ignore
...
def single(request, slug):
    try:
        try:
            ...
            produits = categorie.produit.all() # type: ignore
        except:
            categorie = models.CategorieEtablissement.objects.get(slug=slug)
            produits = categorie.produit_etab.all() # type: ignore
    except:
...
def modifier_article(request, article_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect("modifier", article_id=article.id) # type: ignore
